...
def test_positive_month_basic():
    period = MVPeriod(site_id="S1", period_start_utc="2025-10-01T00:00:00Z", period_end_utc="2025-10-31T23:59:59Z")
    inp = MVInputs(
        baseline_kwh=2400, actual_kwh=1800,
        baseline_units=1000, actual_units=950,
        temp_norm_not_required=True,  # skip temp for pilot
        unload_pct=0.25,
        tariff=dict(weighted_avg_inr_per_kwh=9.5)
    )
